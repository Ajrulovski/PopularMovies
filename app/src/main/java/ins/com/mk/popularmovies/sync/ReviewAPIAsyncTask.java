...
public class ReviewAPIAsyncTask extends AsyncTask<Object, Boolean, String> {
    ...
    protected String doInBackground(Object... params) {
        ...
        try {
            ...
            Uri builtUri = Uri.parse(MOVIES_BASE_URL+id+"/reviews").buildUpon()
            ...
            res = getReviewDataFromJson(moviesJsonStr);
        } catch (IOException e) {
    ...
    private String getReviewDataFromJson(String movieJsonStr)
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                author = row.getString(OMD_AUTHOR);
                content = row.getString(OMD_CONTENT);

                //generate the metadata JSON object
                JSONObject movieObject = new JSONObject();
                movieObject.put("author", author);
                movieObject.put("content", content);
    ...
    protected void onPostExecute(String response) {
        try
        {
            callerActivity.populateReviewList(metadata);
        }
        catch(Exception e)
        {
